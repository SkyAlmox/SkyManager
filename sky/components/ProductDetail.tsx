...
const ProductDetail: React.FC<ProductDetailProps> = ({ product, onBack, onAddToCart }) => {
  ...
  return (
    <div className="pt-24 min-h-screen bg-brand-bg animate-fade-in-up">
      <div className="max-w-[1800px] mx-auto px-6 md:px-12 pb-24">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-24">
          ...
          <div className="flex flex-col justify-center max-w-xl bg-[#F5F5F5] p-8 md:p-12 rounded-sm border border-brand-hover/10 shadow-sm">
             ...
             {showSizes && (
                <div className="mb-8">
                  <span className="block text-xs font-bold uppercase tracking-widest text-brand-dark mb-4">Select Size</span>
                    ...
                    {sizes.map(size => (
                      <button
                        ...
                        className={`w-12 h-12 flex items-center justify-center border transition-all duration-300 ${
                          selectedSize === size
                            ? 'border-brand-dark bg-brand-dark text-[#F5F2EB]'
                            : 'border-brand-dark/20 text-brand-dark/60 hover:border-brand-dark'
                        }`}
                      >
                        {size}
                      </button>
             ...
             <div className="flex flex-col gap-4">
               <button
                 ...
                 className="w-full py-5 bg-brand-dark text-white uppercase tracking-widest text-sm font-medium hover:bg-brand-dark/90 transition-colors"
               >
                 Solicitar Orçamento
               </button>
               <ul className="mt-8 space-y-2 text-sm text-brand-dark/70">
